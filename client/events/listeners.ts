...
export function setupEventListeners(): void {
  ...
  queryForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      lastResult = result;
      setStatus(
        `Result: ${result.label} (${result.latitude.toFixed(6)}, ${result.longitude.toFixed(6)})`,
      );
      appendChatEntry(
        "assistant",
        `${result.message} (${result.latitude}, ${result.longitude})`,
      );
  ...
  placeMarkerButton.addEventListener("click", () => {
    ...
    if (!added) {
      setStatus(
        "Duplicate marker: same place and coordinates already exists.",
        true,
      );
      return;
    }
